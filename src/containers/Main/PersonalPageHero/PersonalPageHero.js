...
import GetHero from "../../../GetData/GetHero/GetHero"
            ...
            { loading ?
                ...
                <div className={classes.PersonalPageHero}>
                    <div className={classes.hero}>
                        <h1>{hero[0].name}</h1>
                        <img className={classes.imgHero} src={hero[0].thumbnail.path + '.' + hero[0].thumbnail.extension} alt={hero[0].name}/>
                        <p>{ hero[0].description ? hero[0].description : 'Description not found' }</p>
                        <div>
                            {hero[0].urls.map((url, index) => {
                                return (
                                    <a key={index} target='_blank' href={url.url}>{url.type}</a>
                                )
                            })}
                        </div>
                    </div>
                    <p className={classes.titleListComics}>Сomics with this hero:</p>
                    <div className={classes.containerComics} style={comicsList.length > 0 ? {justifyContent: "flex-start"}: {justifyContent: "center"}}>
                        {comicsList.length > 0 ?
